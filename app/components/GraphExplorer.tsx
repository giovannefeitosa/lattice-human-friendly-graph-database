...
export default function GraphExplorer({ graph, rootId, onExit }: Props) {
  ...
  const selected = graph.nodes.find((node) => node.id === selectedId) ?? graph.nodes.find((node) => node.id === rootId)!;
  const selectedNeighbors = neighbors(graph, selected.id);
  ...
  return <main className="explore-shell">
    <header className="explore-topbar">
      <button className="explore-back" onClick={onExit}>← Voltar ao gráfico</button>
      ...
      <svg ref={svgRef} className="explore-canvas" role="application" aria-label="Mapa progressivo"
        ...
        <g transform={`translate(${canvasSize.width / 2} ${canvasSize.height / 2}) scale(${zoom}) translate(${pan.x} ${pan.y})`}>
          ...
          {visibleNodes.map((node) => {
            ...
            return <g key={node.id} className={`explore-node${selected.id === node.id ? " selected" : ""}`} transform={`translate(${point.x} ${point.y})`} onPointerDown={(event) => event.stopPropagation()} onClick={() => setSelectedId(node.id)} role="button" aria-label={`${node.label}; ${count} conexões; ${isExpanded ? "aberto" : "fechado"}`}>
              {selected.id === node.id && <circle className="explore-ring" r="56" />}
